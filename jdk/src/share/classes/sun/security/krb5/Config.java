...
public class Config {
    ...
    static {
        String str1 = SecurityProperties.privilegedGetOverridable("sun.security.krb5.disableReferrals");
        if (str1 != null) {
            DISABLE_REFERRALS = "true".equalsIgnoreCase(str1);
        } else {
            DISABLE_REFERRALS = false;
        }
        int i = 5;
        String str2 = SecurityProperties.privilegedGetOverridable("sun.security.krb5.maxReferrals");
        try {
    ...
    public static synchronized Config getInstance() throws KrbException {
        ...
        Config singletonKrb = null;
        if (name != null) {
            singletonKrb = configs.get(name);
            if (singletonKrb == null) {
                singletonKrb = new Config();
                configs.put(name, singletonKrb);
            }
            return singletonKrb;
        }
    ...
    public static synchronized void refresh() throws KrbException {
        String name = getProperty("java.security.krb5.conf");
        if (name != null) {
            Config singletonKrb = new Config();
            configs.put(name, singletonKrb);
            singleton = singletonKrb;
        } else {
            singleton = new Config();
        }
        KdcComm.initStatic();
    ...
    public Boolean getBooleanObject(String... paramVarArgs) {
